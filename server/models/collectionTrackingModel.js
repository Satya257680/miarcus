...
const getPreviousRecipients=async(productId,currentStage,excludeUserId)=>{
 ...
 if(idx<=0){
  ...
  rows=await db.query(`SELECT DISTINCT u.id,u.name,u.email FROM collection_stage_history h
   JOIN users u ON u.id=h.user_id WHERE h.product_id=? AND h.stage_name=? AND u.status='Active' ORDER BY h.id DESC`,
   [productId,prev]);
 }
