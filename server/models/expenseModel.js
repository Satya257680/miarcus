...
const Expense = {
    ...
    async getAll(filters = {}) {
        ...
        if (filters.search) {
            ...
            where.push(`(
                ...
                e.vendor_name LIKE ? OR
                e.expense_type LIKE ? OR
                CAST(e.total_amount AS CHAR) LIKE ? OR
                COALESCE(u.name, '') LIKE ? OR
                COALESCE(u.employee_id, '') LIKE ?
            )`);
    ...
    async getById(id) {
        ...
        return {
            ...
            items,
            checks,
            ai_analysis: safeJson(rows[0].ai_analysis_json, null),
            verification: safeJson(rows[0].verification_json, null)
        };
    },
    ...
    async getTypes() {
        ...
        `);
        return rows.map(row => row.expense_type);
    }
};
