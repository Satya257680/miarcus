...
function TrainingReport() {
    ...
    const printCertificate = (
        ...
        const quizName =
            /^mi\s+arcus\b/i.test(
                String(rawQuizName).trim()
            )
                ? String(rawQuizName).trim()
                : `Mi Arcus ${String(rawQuizName).trim()}`;
        ...
        const verificationLocation =
            ...
            certificateDetail?.latitude !== undefined &&
            certificateDetail?.longitude !== null &&
            certificateDetail?.longitude !== undefined
        ...
        const verificationAccuracy =
            ...
            certificateDetail?.location_accuracy !== undefined
                ? `±${Math.round(Number(certificateDetail.location_accuracy) || 0)}m`
                : "-";
        ...
        const safeVerificationLocation = escapeHtml(verificationLocation);
        const safeVerificationAccuracy = escapeHtml(verificationAccuracy);
        const safeVerificationTime = escapeHtml(verificationTime);
...
.certificate::after {
    ...
    width: 390px;
    height: 420px;
    background: linear-gradient(180deg,#ffb400,#4a3898);
    border-radius: 58% 58% 0 0;
    z-index: 0;
}

.certificate-side-wave {
    position: absolute;
    top: 245px;
    width: 210px;
    height: 430px;
...
.certificate-side-wave.left {
    left: -125px;
    border-radius: 0 70% 0 0;
}

.certificate-side-wave.right {
    right: -125px;
    border-radius: 70% 0 0 0;
}

.certificate-photo-row {
    position: relative;
    z-index: 3;
    display: flex;
...
.certificate-photo {
    ...
    box-shadow: 0 8px 22px rgba(41,34,84,.18);
    background: #f1eff9;
}
...
<div class="certificate">
    ...
    <div class="certificate-side-wave left"></div>
    <div class="certificate-side-wave right"></div>

    <div class="certificate-content">
        <img class="certificate-logo-img" src="${logoUrl}" alt="Mi Arcus logo" />
        ...
        <div class="description">
            For successfully completing the Mi Arcus training assessment and
