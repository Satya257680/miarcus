...
export default function DailyCollectionReport() {
    ...
    const exportCsv = () => {
        ...
        const headers = ["Date", "Store", "Store Code", "Manager", "Status", "Bill Count", "System Billed", "UPI", "Cash", "Bank Transfer", "Card", "Total Collected", "Variance", "Submitted By", "Submitted At"];
        const rows = filteredReports.map((row) => [
            row.report_date, row.store_name, row.store_code, row.manager_name || "", row.status,
                ...
                {loading ? <div className="collection-empty">Loading daily report...</div> : !filteredReports.length ? <div className="collection-empty">No Daily Collection data for this date.</div> : (
                    <div className="daily-report-table-scroll">
                        <table className="daily-report-table">
                            <thead><tr><th>Store</th><th>Manager</th><th>Status</th><th>Bills</th><th>System Total</th><th>UPI</th><th>Cash</th><th>Bank</th><th>Card</th><th>Collected</th><th>Variance</th><th>Submitted By</th><th>Actions</th></tr></thead>
                            <tbody>{filteredReports.map((row) => {
                                ...
                                return <tr key={row.id}>
                                    ...
                                    <td>{row.manager_name || "Not linked"}</td>
