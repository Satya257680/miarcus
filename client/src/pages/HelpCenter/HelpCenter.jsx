...
function HelpCenter({ publicMode = false }) {
    ...
    const [botMessages, setBotMessages] = useState([{ id: "welcome", from: "zarvis", text: `Hi ${effectiveUserName}! I'm Zarvis, your Miarcus Help Assistant. I only use answers approved by your administrator.` }]);
    ...
    const submitBot = async (e) => {
        ...
        try {
            ...
            setBotMessages(m => [...m, { id: `${Date.now()}z`, from: "zarvis", text: data.message, resolved: data.resolved, related: data.related || [] }]);
            setNeedsHuman(!data.resolved);
    ...
    return <div className="help-center-page">
        ...
        {tab==="home" && <>
            ...
            <div className="hc-feature-row"><div><FaRobot/><div><strong>Zarvis answers first</strong><span>Only verified administrator answers are used.</span></div></div><div><FaClock/><div><strong>Always available</strong><span>Self-service help is available 24×7.</span></div></div><div><FaUserTie/><div><strong>Human fallback</strong><span>Open a support request when needed.</span></div></div></div>
        ...
        {tab==="zarvis" && <div className="hc-zarvis"><div className="hc-chat-card"><div className="hc-chat-head"><div className="hc-avatar"><FaRobot/></div><div><strong>Zarvis</strong><span><i/> Verified Miarcus assistant</span></div><span className="hc-24">24×7</span></div><div className="hc-chat-body">{botMessages.map(m=><div key={m.id} className={`hc-msg ${m.from}`}>{m.from==='zarvis'&&<div className="hc-mini-avatar"><FaRobot/></div>}<div className="hc-bubble">{m.text}{m.related?.length>0&&<div className="hc-related"><small>Related verified answers</small>{m.related.map(r=><button key={r.id} onClick={()=>{setBotQuestion(r.question);setTab("zarvis")}}>{r.question}<FaArrowRight/></button>)}</div>}{m.from==='zarvis'&&m.resolved===false&&!publicMode&&<button className="hc-human-btn" onClick={()=>{setTab("support")}}><FaHeadset/> Request human support</button>}</div></div>)}{botBusy&&<div className="hc-msg zarvis"><div className="hc-mini-avatar"><FaRobot/></div><div className="hc-bubble typing"><i/><i/><i/></div></div>}<div ref={chatEndRef}/></div><form className="hc-chat-input" onSubmit={submitBot}><input value={botQuestion} onChange={e=>setBotQuestion(e.target.value)} placeholder="Ask anything about Miarcus…"/><button disabled={botBusy}><FaPaperPlane/></button></form></div><div className="hc-zarvis-side"><div className="hc-trust"><FaShieldAlt/><h3>Verified answers only</h3><p>Zarvis does not invent policy, process or system information. It responds from the Help Center answers your administrator has approved.</p></div><div className="hc-suggest"><span>TRY ASKING</span>{["How do I reset my password?","How can I raise an action point?","Where can I see my reports?"].map(q=><button key={q} onClick={()=>setBotQuestion(q)}>{q}<FaArrowRight/></button>)}</div></div></div>}
