...
// ======================================================
// REJECTED EXPENSE CHECK
// ======================================================
// Rejected expenses remain visible in the register,
// but they must NOT be included in Total Value.

const isRejectedExpense = (expense) => {
...
function hasFullExpenseAccess() {
    try {
        ...
        if (raw && !Array.isArray(raw)) {
            ...
            if (
                raw.Expenses?.permission === "Full"
            ) {
                return true;
            }

            if (
                raw.permissions?.Expenses === "Full"
            ) {
                return true;
            }

            if (
                raw.permissions?.Expenses?.permission === "Full"
            ) {
                return true;
            }
        }
...
function TrackExpenses() {
    ...
    // ==================================================
    // CLIENT-SIDE ACCESS FLAG
    // ==================================================
    // Backend is authoritative. This only acts as a defense-in-depth
    // safeguard so a restricted user never renders another user's row
    // if an old/cached API response is accidentally returned.
    const canViewAllExpenses = hasFullExpenseAccess();
    ...
    const load = async () => {
        try {
            ...
            // Backend already enforces this rule. This extra filter is
            // only a UI safety net for restricted users.
        ...
        () =>
            expenses.reduce(
                (acc, item) => {
                    ...
                    // IMPORTANT:
                    // Rejected expenses are NOT included
                    // in Total Value.
                    if (!isRejectedExpense(item)) {
                        acc.amount += Number(
                            item.total_amount || 0
                ...
                <div className="expense-table-header">
                    ...
                    <div className="expense-table-header-actions">
                        ...
                        */}
                        ...
                        <span className="expense-table-total">
                            {money(visibleTotal)}
                        </span>

                        {canDelete && (
                            <button
                                className="expense-delete-all-btn"
                                onClick={deleteAll}
                                    ...
                                    (item) => (
                                        ...
                                        >
                                            ...
                                            <td>
                                                ...
                                                <span
                                                    className={`expense-status-pill ${statusClass(
                                                        item.status
                                                    )}`}
                                                >
                                                    {
